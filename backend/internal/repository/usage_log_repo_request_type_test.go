...
func TestPrepareUsageLogInsert_ArgCountMatchesTypes(t *testing.T) {
	inputContent := "last human message"
	prepared := prepareUsageLogInsert(&service.UsageLog{
		UserID:         1,
		APIKeyID:       2,
		AccountID:      3,
	...
	require.Equal(t, sql.NullString{String: inputContent, Valid: true}, prepared.args[len(prepared.args)-1])
}
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullFloat64{},
			now,
			sql.NullString{Valid: true, String: "create usage report"}, // input_content
		}})
		require.NoError(t, err)
		...
		require.NotNil(t, log.InputContent)
		require.Equal(t, "create usage report", *log.InputContent)
	})
