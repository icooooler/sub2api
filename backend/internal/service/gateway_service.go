...
type RecordUsageInput struct {
	...
	ParsedRequest      *ParsedRequest     // 可选：解析后的请求体，用于记录 input_content
...
type recordUsageOpts struct {
	// ParsedRequest 用于记录 input_content（可选，仅在调用方持有解析后请求体时传入）
...
func (s *GatewayService) buildRecordUsageLog(
	...
	if opts != nil && opts.ParsedRequest != nil {
		var messages []any
		if err := opts.ParsedRequest.DecodeMessages(&messages); err == nil {
			usageLog.InputContent = ExtractLastUserMessage(messages)
		}
	}
	if result.ImageCount > 0 {
