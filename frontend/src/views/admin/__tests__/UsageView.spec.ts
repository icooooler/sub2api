...
describe('admin UsageView request ID column visibility', () => {
  ...
  it('keeps request ID hidden by default and allows enabling it from column settings', async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith(
      ...
      'request-id-and-input-content-hidden-by-default',
    )
  })
})
...
describe('admin UsageView sensitive column preferences', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    list.mockResolvedValue({ items: [], total: 0, pages: 0 })
    getStats.mockResolvedValue({
  ...
  it('hides input content when migrating legacy column preferences', async () => {
    mount(UsageView, {
      global: { stubs: {
        AppLayout: AppLayoutStub, UsageStatsCards: true, UsageFilters: UsageFiltersStub,
        UsageTable: true, UsageExportProgress: true, UsageCleanupDialog: true,
    ...
    const migrated = vi.mocked(localStorage.setItem).mock.calls.find(
      ([key]) => key === 'usage-hidden-columns-v2'
    )
    expect(migrated).toBeDefined()
    expect(JSON.parse(migrated![1] as string)).toEqual(expect.arrayContaining(['user_agent', 'input_content']))
  })
})
