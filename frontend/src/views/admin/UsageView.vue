...
const DEFAULT_HIDDEN_COLUMNS = ['reasoning_effort', 'request_id', 'upstream_request_id', 'user_agent', 'input_content']
const HIDDEN_COLUMNS_KEY = 'usage-hidden-columns-v2'
const LEGACY_HIDDEN_COLUMNS_KEY = 'usage-hidden-columns'
...
const HIDDEN_COLUMNS_CURRENT_VERSION = 'request-id-and-input-content-hidden-by-default'
  ...
  try {
    ...
    if (saved) {
      ...
      if (savedVersion !== HIDDEN_COLUMNS_CURRENT_VERSION) {
        ...
        // input_content may contain the full user prompt, so existing saved
        // preferences must keep this newly introduced sensitive column hidden.
        hiddenColumns.add('input_content')
        localStorage.setItem(HIDDEN_COLUMNS_KEY, JSON.stringify([...hiddenColumns]))
        localStorage.setItem(HIDDEN_COLUMNS_VERSION_KEY, HIDDEN_COLUMNS_CURRENT_VERSION)
      }
    } else {
      const legacySaved = localStorage.getItem(LEGACY_HIDDEN_COLUMNS_KEY)
      const keys = legacySaved ? (JSON.parse(legacySaved) as string[]) : DEFAULT_HIDDEN_COLUMNS
      keys.forEach((key) => {
        hiddenColumns.add(key)
      })
      // input_content may contain the full user prompt. Existing installations
      // must hide the newly introduced sensitive column during preference migration.
      hiddenColumns.add('request_id')
      hiddenColumns.add('input_content')
      localStorage.setItem(HIDDEN_COLUMNS_KEY, JSON.stringify([...hiddenColumns]))
      localStorage.setItem(HIDDEN_COLUMNS_VERSION_KEY, HIDDEN_COLUMNS_CURRENT_VERSION)
